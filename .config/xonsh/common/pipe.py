...
def _open_file(file_obj: Optional[Union[str, TextIO, BinaryIO]], mode: str, default):
    ...
    if hasattr(file_obj, "read") if "r" in mode else hasattr(file_obj, "write"):
        return file_obj
...
def _stream_stdin_to_proc(proc, chunk_size: int, stdin_file):
...
def pipe_through_subprocess(
    ...
    stdin_chunk_size: int = 1,
    stdout_chunk_size: int = 1,
    stderr_chunk_size: int = 1,
